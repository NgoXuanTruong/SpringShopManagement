...
import com.eshop.repository.service.ProductService;
...
//		model.addAttribute("items", items);
	...
	public String create (Model model, @Validated @ModelAttribute("item") Product item,@RequestPart("image_file") MultipartFile imageFile) {
//		Product item = dao.getById(id);
	...
	public String update (Model model,@Validated @ModelAttribute("item") Product item, @RequestPart("image_file") MultipartFile imageFile ) {
//		
		if(!imageFile.isEmpty()) {
			File image = uploadService.save(imageFile, "/images/items/");
			item.setImage(image.getName());
